...
export default {
  ...
  created() {
    ...
    console.log("详情页的this.receivedParam:", this.receivedParam);
    this.receiveQuery = this.$route.query.imageUrl;
    console.log("详情页的this.receiveQuery:", this.receiveQuery);

    const baseImageUrl = this.receiveQuery;
    console.log("详情baseImageUrl:", baseImageUrl);
    this.detailImgUrl = baseImageUrl.replace(
      "/21.png",
      `/${this.receivedParam}.png`
    );
  ...
  mounted() {
    ...
    const materials = [
      ...
      new THREE.MeshStandardMaterial({
        map: new THREE.TextureLoader().load(this.detailImgUrl), // Front and back face texture
        roughness: 0,
        metalness: 0,
      }),
